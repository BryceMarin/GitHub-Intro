...
API_KEY = "DEMO_KEY"          # NASA's public demo key -- Replace with your own if you have one
...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.DatePickerSingle(
            ...
            clearable=False, # there has to be a date listed
        ),
        # dcc.Loading shows a spinner while the media content is loading
...
@app.callback(
    ...
    # update the contents (children) of the component with id="caption" (the caption placeholder)
    Input("apod-date", "date"),
)
def show_apod(date_str):
    if not date_str:
        raise exceptions.PreventUpdate
    # quick date error check
    try:
    ...
    # if requests don't work error check
    try:
    ...
    # API getting information - uses .get() so a default value is returned if the key is missing
